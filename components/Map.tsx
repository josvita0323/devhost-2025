...
const Map = () => {
  ...
  useEffect(() => {
    const animateCut = (el: HTMLDivElement | null, y: number, duration: number, skew: number) => {
      if (!el) return;
      gsap.to(el, {
        y: y,
        skewY: skew,
  ...
  return (
    <section className="relative min-h-screen w-full bg-black text-white flex items-center justify-center overflow-hidden px-4 sm:px-6">
      ...
      <div
        ref={topCut1}
        className="absolute -top-20 left-0 w-full h-40 sm:h-48 bg-gradient-to-b from-black via-[#a3ff12]/10 to-transparent skew-y-3"
      />
      <div
        ref={topCut2}
        className="absolute -top-10 right-0 w-full h-20 sm:h-24 bg-[#a3ff12]/10 -skew-y-3"
      />

      {/* cuts - bottom */}
      <div
        ref={bottomCut1}
        className="absolute -bottom-20 left-0 w-full h-40 sm:h-48 bg-gradient-to-t from-black via-[#a3ff12]/10 to-transparent -skew-y-3"
      />
      <div
        ref={bottomCut2}
        className="absolute -bottom-10 right-0 w-full h-20 sm:h-24 bg-[#a3ff12]/10 skew-y-3"
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-5 gap-4 items-center py-12">
        <div className="space-y-6 md:col-span-2 text-center md:text-left">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight">
            <span className="text-[#a3ff12] drop-shadow-[0_0_10px_#a3ff12aa] font-orbitron">
              Location
            </span>
          </h2>
        ...
        <div className="relative md:col-span-3 w-full">
          <div
            className="bg-black shadow-[0_0_30px_#a3ff12aa] overflow-hidden transform -skew-y-2 hover:skew-y-0 transition-all duration-500 border-[4px] border-[#a3ff12]"
            style={clipPathStyle}
          >
            <iframe
